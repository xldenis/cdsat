...
fn generate_term(vars: &Generator, depth: usize, sort: Sort) -> String {
    ...
    if depth == 0 {
        if rng.gen() && ((sort == Sort::Bool && vars.bool_vars > 0 ) || (sort == Sort::Real && vars.real_vars > 0 ) ) {
        ...
        if sort == Sort::Bool {
            ...
            match op {
                ...
                4 => {
                    let sort1 = if rng.gen() || vars.real_vars == 0 {
                        Sort::Bool
                    } else {
                        Sort::Real
                    };
...
fn main() {
    ...
    let bool_vars = if args.max_bool > 0 {
        rng.gen_range(0..args.max_bool)
    } else {
        0
    };
    let real_vars = if args.max_real > 0 {
        rng.gen_range(0..args.max_real)
    } else {
        0
    };
